"""
Integration tests for Kling MCP Server.
...
class TestInfoTools:
    ...
    async def test_list_models(self):
        """Test kling_list_models tool."""
        from tools.info_tools import kling_list_models

        result = await kling_list_models()

        print("\n=== List Models Result ===")
        print(result)

        assert "kling" in result.lower()

    @pytest.mark.asyncio
    async def test_list_actions(self):
        """Test kling_list_actions tool."""
        from tools.info_tools import kling_list_actions

        result = await kling_list_actions()

        print("\n=== List Actions Result ===")
        print(result)

        assert "kling_generate_video" in result
        assert "kling_get_task" in result
...
class TestTaskTools:
    ...
    async def test_get_task_with_real_id(self):
        ...
        gen_result = await kling_generate_video(
        ...
        if "Task ID:" in gen_result:
            ...
            if task_id and task_id != "N/A":
                ...
                task_result = await kling_get_task(task_id)
                print(task_result)
