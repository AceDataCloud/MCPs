...
MCP (Model Context Protocol) server for Kling AI video generation via AceDataCloud API.
...
core/
  ...
  types.py      — Literal types (KlingModel, KlingAction, AspectRatio, etc.)
  exceptions.py — Error classes (AuthError, APIError, TimeoutError)
  utils.py      — Formatting helpers
tools/
  video_tools.py   — generate video from text or image, extend video
  motion_tools.py  — motion transfer from video to image
...
1. **Source of truth** — `_docs/openapi/kling.json` is the OpenAPI spec for the KlingMCP API.
