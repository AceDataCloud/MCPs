"""Utility functions for MCP Kling server."""
...
def _with_submission_guidance(
    ...
    payload["mcp_async_submission"] = {
        ...
        "next_step": (
            f'Call {poll_tool}(task_id="{task_id}") to poll until the task '
            f"completes and the final media URLs are available."
        ),
    }
    return payload
...
def format_motion_result(data: dict[str, Any]) -> str:
    """Format motion generation result as JSON.
