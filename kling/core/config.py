"""Configuration management for MCP Kling server."""
...
class Settings:
    ...
    default_model: str = field(
        default_factory=lambda: os.getenv("KLING_DEFAULT_MODEL", "kling-v2-master")
    )
    ...
    default_aspect_ratio: str = field(
        default_factory=lambda: os.getenv("KLING_DEFAULT_ASPECT_RATIO", "16:9")
    )

    # Request Configuration
    request_timeout: float = field(
        default_factory=lambda: float(os.getenv("KLING_REQUEST_TIMEOUT", "1800"))
    )
