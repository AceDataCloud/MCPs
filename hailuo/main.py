#!/usr/bin/env python3
"""
MCP Hailuo Server - AI Video Generation via AceDataCloud API.

A Model Context Protocol (MCP) server that provides tools for generating
AI videos using Hailuo (MiniMax) through the AceDataCloud platform.
"""
...
def safe_print(text: str) -> None:
    ...
    if not sys.stderr.isatty():
        logger.debug(f"[MCP Hailuo] {text}")
        return
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-hailuo")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Hailuo server."""
    parser = argparse.ArgumentParser(
        description="MCP Hailuo Server - AI Video Generation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-hailuo                    # Run with stdio transport (default)
  mcp-hailuo --transport http   # Run with HTTP transport
  mcp-hailuo --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN     API token from AceDataCloud (required)
  HAILUO_DEFAULT_MODEL       Default video model (default: minimax-t2v)
  HAILUO_REQUEST_TIMEOUT     Request timeout in seconds (default: 1800)
  LOG_LEVEL                  Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-hailuo {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP Hailuo Server - AI Video Generation")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - hailuo_generate_video")
    safe_print("    - hailuo_generate_video_from_image")
    safe_print("    - hailuo_get_task")
    safe_print("    - hailuo_get_tasks_batch")
    safe_print("    - hailuo_list_models")
    safe_print("    - hailuo_list_actions")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - hailuo_video_generation_guide")
    safe_print("    - hailuo_workflow_examples")
    safe_print("    - hailuo_prompt_suggestions")
    safe_print("")
    safe_print("=" * 50)
                ...
                return JSONResponse(
                    {
                        ...
                        "prompts": [
                            ...
                            {
                                "name": "hailuo_workflow_examples",
                                "description": "Example workflows",
                            },
                            {
                                "name": "hailuo_prompt_suggestions",
