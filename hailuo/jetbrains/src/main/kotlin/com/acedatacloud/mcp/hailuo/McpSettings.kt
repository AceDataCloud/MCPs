package com.acedatacloud.mcp.hailuo
...
@State(name = "McpSettings_hailuo", storages = [Storage("McpHailuoSettings.xml")])
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    fun getStdioConfig(): String {
        ...
        return """{"mcpServers": {"hailuo": {"command": "uvx", "args": ["mcp-hailuo"], "env": {"ACEDATACLOUD_API_TOKEN": "$token"}}}}"""
    }

    fun getHttpConfig(): String {
        val token = myState.apiToken.ifEmpty { "YOUR_API_TOKEN" }
        return """{"mcpServers": {"hailuo": {"url": "https://hailuo.mcp.acedata.cloud/mcp", "headers": {"Authorization": "Bearer $token"}}}}"""
    }
}
