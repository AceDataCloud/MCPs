"""
Integration tests for Hailuo MCP Server.
...
class TestVideoTools:
    ...
    async def test_generate_video_basic(self):
        ...
        from tools.video_tools import hailuo_generate_video

        result = await hailuo_generate_video(
...
class TestInfoTools:
    ...
    async def test_list_models(self):
        """Test hailuo_list_models tool."""
        from tools.info_tools import hailuo_list_models

        result = await hailuo_list_models()
    ...
    async def test_list_actions(self):
        """Test hailuo_list_actions tool."""
        from tools.info_tools import hailuo_list_actions

        result = await hailuo_list_actions()

        print("\n=== List Actions Result ===")
        print(result)

        assert "hailuo_generate_video" in result
        assert "hailuo_get_task" in result
...
class TestTaskTools:
    ...
    async def test_get_task_with_real_id(self):
        ...
        gen_result = await hailuo_generate_video(
        ...
        if "Task ID:" in gen_result:
            ...
            if task_id and task_id != "N/A":
                ...
                task_result = await hailuo_get_task(task_id)
                print(task_result)
