...
//
// Registers the hosted Ace Data Cloud "acedatacloud" MCP server with VS Code
// via the stable `vscode.lm.registerMcpServerDefinitionProvider` API. The
// Bearer API key is read from (in order):
//   1. process.env.ACEDATACLOUD_PLATFORM_TOKEN
//   2. VS Code SecretStorage (key "acedatacloud.apiToken")
...
//   - acedatacloud.acedatacloud.setApiToken
//   - acedatacloud.acedatacloud.clearApiToken
...
const SECRET_KEY = "acedatacloud.apiToken";
const CREDENTIAL_URL = "https://platform.acedata.cloud/console/platform-tokens?utm_source=vscode&utm_medium=extension&utm_campaign=vscode-acedatacloud";

async function readToken(context) {
  const env = process.env.ACEDATACLOUD_PLATFORM_TOKEN;
...
async function promptForToken(context) {
  const token = await vscode.window.showInputBox({
    title: `${SERVER_LABEL} — Ace Data Cloud API key`,
        prompt: `Paste an API key from ${CREDENTIAL_URL} (Platform Tokens -> Create). Stored in the OS keychain.`,
