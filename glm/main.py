#!/usr/bin/env python3
"""
MCP GLM Server - Zhipu GLM Chat Completions via AceDataCloud.

A Model Context Protocol (MCP) server that provides tools for interacting with
Zhipu GLM models through the AceDataCloud platform.
"""
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-glm")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP GLM server."""
    parser = argparse.ArgumentParser(
        description="MCP GLM Server - Zhipu GLM Chat Completions via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-glm                    # Run with stdio transport (default)
  mcp-glm --transport http   # Run with HTTP transport
  mcp-glm --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN      API token from AceDataCloud (required)
  GLM_REQUEST_TIMEOUT         Request timeout in seconds (default: 60)
  LOG_LEVEL                   Logging level (default: INFO)
    ...
    safe_print("    - glm_chat_completions")
    safe_print("    - glm_list_models")
    safe_print("    - glm_get_usage_guide")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - glm_guide")
    safe_print("    - glm_workflow_examples")
    safe_print("")
    safe_print("=" * 50)
                ...
                return JSONResponse(
                    {
                        ...
                        "prompts": [
                            ...
                            {
                                "name": "glm_workflow_examples",
                                "description": "Example GLM workflows",
                            },
                        ],
                        "resources": [],
                    }
