...
class AceDataCloudOAuthProvider:
    ...
    In-memory storage is used for auth state (suitable for single-replica
    K8s deployment).
    """
    ...
    async def authorize(
        ...
        auth_url = (
            f"{settings.auth_base_url}/oauth2/authorize?"
            f"{urlencode(auth_params)}"
        )
        logger.info(
            f"OAuth authorize: redirecting to consent page "
            f"(mcp_state={mcp_state})"
        )
        return auth_url
    ...
    async def handle_callback(
        ...
        This is called as a Starlette route handler, not part of the SDK
        interface.
        """
        ...
        try:
            ...
            jwt_token = await self._exchange_code_for_jwt(
                adc_code, code_verifier
            )
            if not jwt_token:
                return JSONResponse(
            ...
            if not api_token:
                return JSONResponse(
                    {
                        "error": (
                            "No API credential found. Please create an API "
                            "key at https://platform.acedata.cloud first."
                        )
                    },
                    status_code=403,
                )
    ...
    async def exchange_authorization_code(
        ...
        logger.info(
            f"OAuth token exchange: issued access token for client "
            f"{client_id}"
        )
        return OAuthToken(
    ...
    async def load_access_token(self, token: str) -> AccessToken | None:
        ...
        Direct tokens are accepted since the real validation happens at
        api.acedata.cloud.
        """
    ...
    async def _get_user_credential(
        ...
        """Fetch or auto-create user's API credential token.
        ...
        4. Create credential under that application
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as http_client:
                ...
                if not application_id:
                    ...
                    create_app_resp = await http_client.post(
                        f"{settings.platform_base_url}/api/v1/applications/",
                        headers={
                            **headers,
                    ...
                    if create_app_resp.status_code in (200, 201):
                        ...
                        logger.error(
                            ...
                            f"{create_app_resp.status_code} "
                            f"{create_app_resp.text}"
                        )
                        return None

                # Step 2b: Create a credential under the application
                cred_resp = await http_client.post(
                    f"{settings.platform_base_url}/api/v1/credentials/",
                    headers={
                        **headers,
                        "Content-Type": "application/json",
