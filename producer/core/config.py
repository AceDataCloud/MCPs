"""Configuration management for MCP Producer server."""
...
class Settings:
    ...
    default_model: str = field(
        default_factory=lambda: os.getenv("PRODUCER_DEFAULT_MODEL", "FUZZ-2.0")
    )

    # Request Configuration
    request_timeout: float = field(
        default_factory=lambda: float(os.getenv("PRODUCER_REQUEST_TIMEOUT", "1800"))
    )

    # Server Configuration
    server_name: str = field(
        default_factory=lambda: os.getenv("MCP_SERVER_NAME", "producer")
    )
    transport: str = field(
