...
3. **Open Copilot Chat** in agent mode and ask for a music task — the extension prompts for the token the first time and stores it in the OS keychain via VS Code's `SecretStorage`.

You can rotate or remove the token any time from the command palette:

- **Producer MCP: Set Ace Data Cloud API Token**
- **Producer MCP: Clear Ace Data Cloud API Token**
