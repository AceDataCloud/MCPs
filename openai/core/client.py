...
class OpenaiClient:
    ...
    async def chat_completion(self, **kwargs: Any) -> dict[str, Any]:
        ...
        logger.info(f"Chat completion with model: {kwargs.get('model', '')}")
        return await self.request("/openai/chat/completions", kwargs)

    async def embeddings(self, **kwargs: Any) -> dict[str, Any]:
        """Create embeddings for input text."""
        logger.info(f"Embeddings with model: {kwargs.get('model', '')}")
        return await self.request("/openai/embeddings", kwargs)

    async def image_generate(self, **kwargs: Any) -> dict[str, Any]:
        """Generate images from a prompt."""
        logger.info(f"Image generation with model: {kwargs.get('model', '')}")
        return await self.request("/openai/images/generations", kwargs)

    async def responses(self, **kwargs: Any) -> dict[str, Any]:
        """Create a response using the responses API."""
        logger.info(f"Responses API with model: {kwargs.get('model', '')}")
        return await self.request("/openai/responses", kwargs)

    async def image_edit(self, **kwargs: Any) -> dict[str, Any]:
        """Edit an existing image."""
        logger.info(f"Image edit with model: {kwargs.get('model', '')}")
