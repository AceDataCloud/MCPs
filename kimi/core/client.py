"""HTTP client for Kimi API via AceDataCloud."""
...
from core.exceptions import KimiAPIError, KimiAuthError, KimiError, KimiTimeoutError
...
class KimiClient:
    """Async HTTP client for AceDataCloud Kimi API."""
    ...
    def __init__(self, api_token: str | None = None, base_url: str | None = None):
        ...
        logger.info(f"KimiClient initialized with base_url: {self.base_url}")
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        if not token:
            ...
            raise KimiAuthError("API token not configured")
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if status in (401, 403):
            raise KimiAuthError(message)
        raise KimiAPIError(message=message, code=code, status_code=status)
    ...
    async def request(
        ...
        Args:
            endpoint: API endpoint path (e.g., "/kimi/chat/completions")
        ...
        Raises:
            KimiAuthError: If authentication fails
            KimiAPIError: If the API request fails
            KimiTimeoutError: If the request times out
        """
    ...
    async def chat_completions(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return await self.request("/kimi/chat/completions", kwargs)
