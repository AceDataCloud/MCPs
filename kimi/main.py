#!/usr/bin/env python3
"""
MCP Kimi Server - Kimi API via AceDataCloud.

A Model Context Protocol (MCP) server that provides tools for interacting with
Kimi models (chat completions and more) through the AceDataCloud platform.
"""
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-kimi")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Kimi server."""
    parser = argparse.ArgumentParser(
        description="MCP Kimi Server - Kimi API via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-kimi                    # Run with stdio transport (default)
  mcp-kimi --transport http   # Run with HTTP transport
  mcp-kimi --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN      API token from AceDataCloud (required)
  KIMI_REQUEST_TIMEOUT      Request timeout in seconds (default: 60)
  LOG_LEVEL                   Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-kimi {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("    - kimi_chat_completion")
    safe_print("    - kimi_list_models")
    safe_print("    - kimi_get_usage_guide")
    safe_print("")
    safe_print("=" * 50)
    ...
    try:
        if args.transport == "http":
            ...
            async def server_card(_request: Request) -> JSONResponse:
                ...
                return JSONResponse(
                    {
                        "serverInfo": {"name": "MCP Kimi"},
                        "authentication": {"required": True, "schemes": ["bearer"]},
                        "tools": [
                            {
                                "name": "kimi_chat_completion",
                                "description": "Create chat completions using Kimi models",
                            },
                            {
                            ...
                            {
                                "name": "kimi_get_usage_guide",
                                "description": "Get Kimi API usage guide",
                            },
                        ],
                        "prompts": [],
                        "resources": [],
