#!/usr/bin/env python3
"""
MCP Claude Server - Claude API via AceDataCloud.

A Model Context Protocol (MCP) server that provides tools for interacting with
Claude models (chat completions and more) through the AceDataCloud platform.
"""
...
def safe_print(text: str) -> None:
    ...
    if not sys.stderr.isatty():
        logger.debug(f"[MCP Claude] {text}")
        return
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-claude")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Claude server."""
    parser = argparse.ArgumentParser(
        description="MCP Claude Server - Claude API via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-claude                    # Run with stdio transport (default)
  mcp-claude --transport http   # Run with HTTP transport
  mcp-claude --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN      API token from AceDataCloud (required)
  CLAUDE_REQUEST_TIMEOUT      Request timeout in seconds (default: 60)
  LOG_LEVEL                   Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-claude {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("    - claude_chat_completion")
    safe_print("    - claude_create_message")
    safe_print("    - claude_list_models")
    safe_print("    - claude_get_usage_guide")
    safe_print("")
    safe_print("=" * 50)
    ...
    try:
        if args.transport == "http":
            ...
            async def server_card(_request: Request) -> JSONResponse:
                ...
                return JSONResponse(
                    {
                        "serverInfo": {"name": "MCP Claude"},
                        "authentication": {"required": True, "schemes": ["bearer"]},
                        "tools": [
                            {
                                "name": "claude_chat_completion",
                                "description": "Create chat completions using Claude models",
                            },
                            {
                                "name": "claude_create_message",
                                "description": "Create a message using the Anthropic native Claude API",
                            },
                            {
                            ...
                            {
                                "name": "claude_get_usage_guide",
                                "description": "Get Claude API usage guide",
                            },
                        ],
                        "prompts": [],
                        "resources": [],
