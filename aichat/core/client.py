"""HTTP client for AIChat API via AceDataCloud."""
...
class AIChatClient:
    ...
    async def request(
        ...
        Args:
            endpoint: API endpoint path (e.g., "/aichat/conversations")
        ...
        Raises:
            AIChatAuthError: If authentication fails
            AIChatAPIError: If the API request fails
            AIChatTimeoutError: If the request times out
        """
    ...
    async def conversations(self, **kwargs: Any) -> dict[str, Any]:
        """Send a conversation request to the AIChat API."""
        logger.info(f"AIChat conversation with model: {kwargs.get('model', 'unknown')}")
        return await self.request("/aichat/conversations", kwargs)
