#!/usr/bin/env python3
"""
MCP AIChat Server - AI Dialogue via AceDataCloud API.

A Model Context Protocol (MCP) server that provides tools for conversing with
large language models using the AIChat API through the AceDataCloud platform.
"""
...
Environment Variables:
  ...
  LOG_LEVEL                  Logging level (default: INFO)
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP AIChat Server - AI Dialogue")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - aichat_conversation")
    safe_print("    - aichat_list_models")
    safe_print("    - aichat_get_usage_guide")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - aichat_usage_guide")
    safe_print("    - aichat_model_selection_guide")
    safe_print("")
    safe_print("=" * 50)
                ...
                return JSONResponse(
                    {
                        ...
                        "tools": [
                            {
                                "name": "aichat_conversation",
                                "description": "Send a question to an AI model and get an answer",
                            },
                            {
                                "name": "aichat_list_models",
                                "description": "List available AI chat models",
                            },
                            {
                                "name": "aichat_get_usage_guide",
                                "description": "Get usage guide for AIChat tools",
                            },
                        ],
                        "prompts": [
                            {
                                "name": "aichat_usage_guide",
                                "description": "Guide for using AIChat conversation tool",
                            },
                            {
                                "name": "aichat_model_selection_guide",
                                "description": "Guide for selecting the right AI model",
                            },
                        ],
                        "resources": [],
                    }
