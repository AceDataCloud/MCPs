#!/usr/bin/env python3
"""MCP Localization Server - Translation via AceDataCloud."""
...
def safe_print(text: str) -> None:
    if not sys.stderr.isatty():
        logger.debug(f"[MCP Localization] {text}")
        return
    try:
...
def get_version() -> str:
    try:
        return metadata.version("mcp-localization")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MCP Localization Server - Translation via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--version", action="version", version=f"mcp-localization {get_version()}")
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP Localization Server")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - localization_translate")
    safe_print("    - localization_get_usage_guide")
    safe_print("    - localization_get_api_info")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - localization_guide")
    safe_print("    - localization_workflow_examples")
    safe_print("")
    safe_print("=" * 50)
    ...
    try:
        if args.transport == "http":
            ...
            async def server_card(_request: Request) -> JSONResponse:
                return JSONResponse(
                    {
                        "serverInfo": {"name": "MCP Localization"},
                        "authentication": {"required": True, "schemes": ["bearer"]},
                        "tools": [
                            {
                                "name": "localization_translate",
                                "description": "Translate Markdown or JSON localization input",
                            },
                            {
                                "name": "localization_get_usage_guide",
                                "description": "Get localization usage guide",
                            },
                            {
                                "name": "localization_get_api_info",
                                "description": "Get localization API information",
                            },
                        ],
                        "prompts": [
                            {
                                "name": "localization_guide",
                                "description": "Localization tool selection guide",
                            },
                            {
                                "name": "localization_workflow_examples",
                                "description": "Example localization workflows",
                            },
                        ],
                        "resources": [],
                    }
