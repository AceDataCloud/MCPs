...
@State(name = "McpSettings_sora", storages = [Storage("McpSoraSettings.xml")])
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    fun getStdioConfig(): String {
        ...
        return """{"mcpServers": {"sora": {"command": "uvx", "args": ["mcp-sora"], "env": {"ACEDATACLOUD_API_TOKEN": "$$token"}}}}"""
    }

    fun getHttpConfig(): String {
        val token = myState.apiToken.ifEmpty { "YOUR_API_TOKEN" }
        return """{"mcpServers": {"sora": {"url": "https://sora.mcp.acedata.cloud/mcp", "headers": {"Authorization": "Bearer $$token"}}}}"""
    }
}
