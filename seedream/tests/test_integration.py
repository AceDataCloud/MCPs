"""
Integration tests for Seedream MCP Server.
...
class TestImageTools:
    ...
    async def test_generate_image_basic(self) -> None:
        ...
        from tools.image_tools import seedream_generate_image
...
class TestInfoTools:
    ...
    @pytest.mark.asyncio
    async def test_list_sizes(self) -> None:
        """Test seedream_list_sizes tool."""
        from tools.info_tools import seedream_list_sizes

        result = await seedream_list_sizes()
...
class TestTaskTools:
    ...
    async def test_get_task_with_real_id(self) -> None:
        ...
        gen_result = await seedream_generate_image(
        ...
        if task_id:
            # Query the task
            task_result = await seedream_get_task(task_id=task_id)
            print("\n=== Task Result ===")
            print(task_result)
