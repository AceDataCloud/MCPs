package com.acedatacloud.mcp.fish
...
@State(name = "McpSettings_fish", storages = [Storage("McpFishSettings.xml")])
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    fun getStdioConfig(): String {
        ...
        return """{"mcpServers": {"fish": {"command": "uvx", "args": ["mcp-fish"], "env": {"ACEDATACLOUD_API_TOKEN": "$token"}}}}"""
    }

    fun getHttpConfig(): String {
        val token = myState.apiToken.ifEmpty { "YOUR_API_TOKEN" }
        return """{"mcpServers": {"fish": {"url": "https://fish.mcp.acedata.cloud/mcp", "headers": {"Authorization": "Bearer $token"}}}}"""
    }
}
