#!/usr/bin/env python3
"""
MCP Fish Server - AI Audio Generation via AceDataCloud API.

A Model Context Protocol (MCP) server that provides tools for generating
AI audio (text-to-speech and voice cloning) using Fish through the AceDataCloud platform.
"""
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-fish")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Fish server."""
    parser = argparse.ArgumentParser(
        description="MCP Fish Server - AI Audio Generation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-fish                    # Run with stdio transport (default)
  mcp-fish --transport http   # Run with HTTP transport
  mcp-fish --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN     API token from AceDataCloud (required)
  FISH_DEFAULT_MODEL         Default TTS model (default: fish-tts)
  ...
  LOG_LEVEL                  Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-fish {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP Fish Server - AI Audio Generation")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - fish_generate_audio")
    safe_print("    - fish_create_voice")
    safe_print("    - fish_get_task")
    safe_print("    - fish_get_tasks_batch")
    safe_print("    - fish_list_models")
    safe_print("    - fish_list_actions")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - fish_audio_generation_guide")
    safe_print("    - fish_workflow_examples")
    safe_print("    - fish_prompt_suggestions")
    safe_print("")
    safe_print("=" * 50)
                ...
                return JSONResponse(
                    {
                        ...
                        "tools": [
                            ...
                            },
                            {
                                ...
                                "description": "Create a custom voice from audio URL",
                            },
                            {"name": "fish_get_task", "description": "Query task status"},
                            {
                            ...
                            {
                                "name": "fish_list_actions",
                                "description": "List available actions",
                            },
                        ],
                        "prompts": [
                            {
                                "name": "fish_audio_generation_guide",
                                "description": "Guide for audio generation",
                            },
                            {
                                "name": "fish_workflow_examples",
                                "description": "Example workflows",
                            },
                            {
                                "name": "fish_prompt_suggestions",
