...
from core.exceptions import FishAPIError, FishAuthError, FishTimeoutError


@pytest.fixture
def client():
    """Create a client instance for testing."""
    return FishClient(api_token="test-token", base_url="https://api.test.com")
...
class TestFishClient:
    ...
    def test_init_with_params(self):
        ...
        client = FishClient(api_token="my-token", base_url="https://custom.api.com")
    ...
    def test_get_headers_no_token(self):
        ...
        client = FishClient(api_token="", base_url="https://api.test.com")
        with pytest.raises(FishAuthError, match="not configured"):
            client._get_headers()
    ...
    def test_with_async_callback_injects_default_callback(self, client):
        ...
        payload = client._with_async_callback({"action": "speech"})
    ...
    def test_with_async_callback_preserves_explicit_callback(self, client):
        ...
        payload = client._with_async_callback(
            {"action": "speech", "callback_url": "https://example.com/webhook"}
        )
    ...
    async def test_request_success(self, client, mock_audio_response):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            result = await client.request("/fish/audios", {"action": "speech"})
    ...
    async def test_request_auth_error_401(self, client):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with pytest.raises(FishAuthError, match="Invalid API token"):
                await client.request("/fish/audios", {})
    ...
    async def test_request_timeout(self, client):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with pytest.raises(FishTimeoutError, match="timed out"):
                await client.request("/fish/audios", {})
    ...
    async def test_request_http_error(self, client):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with pytest.raises(FishAPIError, match="Internal Server Error") as exc_info:
                await client.request("/fish/audios", {})
