...
class FishClient:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if result.get("success"):
                    ...
                    if data and isinstance(data, list) and data[0].get("audio_url"):
                        logger.info(f"Audio URL: {data[0].get('audio_url')}")
                else:
    ...
    async def generate_audio(self, **kwargs: Any) -> dict[str, Any]:
        ...
        logger.info(f"Generating audio with model: {kwargs.get('model', 'fish-tts')}")
    ...
    async def create_voice(self, **kwargs: Any) -> dict[str, Any]:
        """Create a voice using the voices endpoint."""
        logger.info(f"Creating voice from: {kwargs.get('voice_url', '')[:50]}...")
