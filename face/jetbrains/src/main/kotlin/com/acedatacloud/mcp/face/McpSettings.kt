package com.acedatacloud.mcp.face
...
@State(name = "McpSettings_face", storages = [Storage("McpFaceSettings.xml")])
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    fun getStdioConfig(): String {
        ...
        return """{"mcpServers": {"face": {"command": "uvx", "args": ["mcp-face-transform"], "env": {"ACEDATACLOUD_API_TOKEN": "$token"}}}}"""
    }

    fun getHttpConfig(): String {
        val token = myState.apiToken.ifEmpty { "YOUR_API_TOKEN" }
        return """{"mcpServers": {"face": {"url": "https://face.mcp.acedata.cloud/mcp", "headers": {"Authorization": "Bearer $token"}}}}"""
    }
}
