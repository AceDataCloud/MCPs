#!/usr/bin/env python3
"""
MCP Minimax Server - AI Video Generation via AceDataCloud API.

A Model Context Protocol (MCP) server that provides tools for generating
AI videos using MiniMax H3 through the AceDataCloud platform.
"""
...
def safe_print(text: str) -> None:
    ...
    if not sys.stderr.isatty():
        logger.debug(f"[MCP Minimax] {text}")
        return
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-minimax")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Minimax server."""
    parser = argparse.ArgumentParser(
        description="MCP Minimax Server - AI Video Generation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-minimax                    # Run with stdio transport (default)
  mcp-minimax --transport http   # Run with HTTP transport
  mcp-minimax --version          # Show version
...
Environment Variables:
  ...
  LOG_LEVEL                  Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-minimax {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP Minimax Server - AI Video Generation")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - minimax_video_generation_guide")
    safe_print("    - minimax_workflow_examples")
    safe_print("    - minimax_prompt_suggestions")
    safe_print("")
    safe_print("=" * 50)
                ...
                return JSONResponse(
                    {
                        ...
                        "prompts": [
                            ...
                            {
                                "name": "minimax_workflow_examples",
                                "description": "Example workflows",
                            },
                            {
                                "name": "minimax_prompt_suggestions",
