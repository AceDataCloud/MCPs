...
MCP (Model Context Protocol) server for Grok Imagine AI video generation via AceDataCloud API.
...
core/
  ...
  types.py      — Literal types (GrokVideoModel, etc.)
  exceptions.py — Error classes (GrokAuthError, GrokAPIError, GrokTimeoutError)
  utils.py      — Formatting helpers
tools/
  video_tools.py   — generate video from text or image
  task_tools.py    — query task status, batch query
  info_tools.py    — list models, actions, prompt guide
...
- `POST /grok/videos` — text-to-video and image-to-video generation
- `POST /grok/tasks` — query task status (async polling)

## Sync from Docs

When invoked by the sync workflow, the Docs repo is checked out at `_docs/`. Your job:

1. **Source of truth** — `_docs/openapi/grok.json` is the OpenAPI spec for the Grok video API.
