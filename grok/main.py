#!/usr/bin/env python3
"""
MCP Grok Server - AI Video Generation via AceDataCloud API.

A Model Context Protocol (MCP) server that provides tools for generating
AI video using Grok Imagine through the AceDataCloud platform.
"""
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-grok")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Grok server."""
    parser = argparse.ArgumentParser(
        description="MCP Grok Server - AI Video Generation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-grok                    # Run with stdio transport (default)
  mcp-grok --transport http   # Run with HTTP transport
  mcp-grok --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN     API token from AceDataCloud (required)
  GROK_DEFAULT_MODEL         Default model (default: grok-imagine-video)
  GROK_REQUEST_TIMEOUT       Request timeout in seconds (default: 180)
  LOG_LEVEL                  Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-grok {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP Grok Server - AI Video Generation")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - grok_get_tasks_batch")
    safe_print("    - grok_list_models")
    safe_print("    - grok_list_actions")
    safe_print("    - grok_get_prompt_guide")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - grok_video_generation_guide")
    safe_print("    - grok_workflow_examples")
    safe_print("    - grok_style_suggestions")
    safe_print("")
    safe_print("=" * 50)
                ...
                return JSONResponse(
                    {
                        ...
                        "prompts": [
                            ...
                            {"name": "grok_workflow_examples", "description": "Example workflows"},
                            {"name": "grok_style_suggestions", "description": "Style suggestions"},
                        ],
                        "resources": [],
                    }
                )
