"""HTTP client for Grok Imagine video API."""
...
class GrokClient:
    """Async HTTP client for AceDataCloud Grok Imagine video API."""
    ...
    def __init__(self, api_token: str | None = None, base_url: str | None = None):
        ...
        logger.info(f"GrokClient initialized with base_url: {self.base_url}")
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        if not token:
            ...
            raise GrokAuthError("API token not configured")
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if status in (401, 403):
            raise GrokAuthError(message)
        raise GrokAPIError(message=message, code=code, status_code=status)
    ...
    async def request(
        ...
        Args:
            endpoint: API endpoint path (e.g., "/grok/videos")
        ...
        Raises:
            ...
            GrokTimeoutError: If the request times out
        """
    ...
    async def generate_video(self, **kwargs: Any) -> dict[str, Any]:
        """Generate video using the Grok videos endpoint."""
        logger.info(f"🎬 Generating grok video with model: {kwargs.get('model', 'default')}")
        return await self.request("/grok/videos", self._with_async_callback(kwargs))
