package com.acedatacloud.mcp.grok
...
@State(name = "McpSettings_grok", storages = [Storage("McpGrokSettings.xml")])
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    fun getStdioConfig(): String {
        ...
        return """{"mcpServers": {"grok": {"command": "uvx", "args": ["mcp-grok"], "env": {"ACEDATACLOUD_API_TOKEN": "$$token"}}}}"""
    }

    fun getHttpConfig(): String {
        val token = myState.apiToken.ifEmpty { "YOUR_API_TOKEN" }
        return """{"mcpServers": {"grok": {"url": "https://grok.mcp.acedata.cloud/mcp", "headers": {"Authorization": "Bearer $$token"}}}}"""
    }
}
