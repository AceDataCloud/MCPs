...
class TestGeminiClient:
    ...
    async def test_chat_completions_method(self, client, mock_chat_response):
        ...
        with patch("httpx.AsyncClient") as mock_http:
            ...
            assert "/gemini/chat/completions" in call_args[0][0]

    @pytest.mark.asyncio
    async def test_generate_content_method(self, client, mock_chat_response):
        """Test the generate_content convenience method."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = mock_chat_response
        ...
        with patch("httpx.AsyncClient") as mock_http:
            ...
            call_args = mock_instance.post.call_args
            assert "/v1beta/models/gemini-2.5-pro:generateContent" in call_args[0][0]
