#!/usr/bin/env python3
"""
MCP Gemini Server - Gemini API via AceDataCloud.

A Model Context Protocol (MCP) server that provides tools for interacting with
Gemini models (chat completions and more) through the AceDataCloud platform.
"""
...
def safe_print(text: str) -> None:
    ...
    if not sys.stderr.isatty():
        logger.debug(f"[MCP Gemini] {text}")
        return
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-gemini")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP Gemini server."""
    parser = argparse.ArgumentParser(
        description="MCP Gemini Server - Gemini API via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-gemini                    # Run with stdio transport (default)
  mcp-gemini --transport http   # Run with HTTP transport
  mcp-gemini --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN      API token from AceDataCloud (required)
  GEMINI_REQUEST_TIMEOUT      Request timeout in seconds (default: 60)
  LOG_LEVEL                   Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-gemini {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("    - gemini_chat_completion")
    safe_print("    - gemini_generate_content")
    safe_print("    - gemini_list_models")
    safe_print("    - gemini_get_usage_guide")
    safe_print("")
    safe_print("=" * 50)
    ...
    try:
        if args.transport == "http":
            ...
            async def server_card(_request: Request) -> JSONResponse:
                ...
                return JSONResponse(
                    {
                        "serverInfo": {"name": "MCP Gemini"},
                        "authentication": {"required": True, "schemes": ["bearer"]},
                        "tools": [
                            {
                                "name": "gemini_chat_completion",
                                "description": "Create chat completions using Gemini models",
                            },
                            {
                                "name": "gemini_generate_content",
                                "description": "Generate content using the Google native Gemini API",
                            },
                            {
                            ...
                            {
                                "name": "gemini_get_usage_guide",
                                "description": "Get Gemini API usage guide",
                            },
                        ],
                        "prompts": [],
                        "resources": [],
