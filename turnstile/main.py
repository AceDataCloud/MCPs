#!/usr/bin/env python3
"""MCP Turnstile Server - Captcha solving via AceDataCloud."""
...
def safe_print(text: str) -> None:
    if not sys.stderr.isatty():
        logger.debug(f"[MCP Turnstile] {text}")
        return
    try:
...
def get_version() -> str:
    try:
        return metadata.version("mcp-turnstile")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MCP Turnstile Server - Captcha solving via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-turnstile                    # Run with stdio transport (default)
  mcp-turnstile --transport http   # Run with HTTP transport
  mcp-turnstile --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN     API token from AceDataCloud (required)
  TURNSTILE_REQUEST_TIMEOUT   Request timeout in seconds (default: 120)
  LOG_LEVEL                  Logging level (default: INFO)
        """,
    )
    parser.add_argument("--version", action="version", version=f"mcp-turnstile {get_version()}")
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP Turnstile Server")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - turnstile_guide")
    safe_print("    - turnstile_workflow_examples")
    safe_print("")
    safe_print("=" * 50)
