#!/usr/bin/env python3
"""
MCP DeepSeek Server - DeepSeek API via AceDataCloud.

A Model Context Protocol (MCP) server that provides tools for interacting with
DeepSeek models (chat completions and more) through the AceDataCloud platform.
"""
...
def safe_print(text: str) -> None:
    ...
    if not sys.stderr.isatty():
        logger.debug(f"[MCP DeepSeek] {text}")
        return
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-deepseek")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP DeepSeek server."""
    parser = argparse.ArgumentParser(
        description="MCP DeepSeek Server - DeepSeek API via AceDataCloud",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-deepseek                    # Run with stdio transport (default)
  mcp-deepseek --transport http   # Run with HTTP transport
  mcp-deepseek --version          # Show version

Environment Variables:
  ACEDATACLOUD_API_TOKEN      API token from AceDataCloud (required)
  DEEPSEEK_REQUEST_TIMEOUT      Request timeout in seconds (default: 60)
  LOG_LEVEL                   Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-deepseek {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP DeepSeek Server")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - deepseek_chat_completion")
    safe_print("    - deepseek_list_models")
    safe_print("    - deepseek_get_usage_guide")
    safe_print("")
    safe_print("=" * 50)
    ...
    try:
        if args.transport == "http":
            ...
            async def server_card(_request: Request) -> JSONResponse:
                ...
                return JSONResponse(
                    {
                        "serverInfo": {"name": "MCP DeepSeek"},
                        "authentication": {"required": True, "schemes": ["bearer"]},
                        "tools": [
                            {
                                "name": "deepseek_chat_completion",
                                "description": "Create chat completions using DeepSeek models",
                            },
                            {
                            ...
                            {
                                "name": "deepseek_get_usage_guide",
                                "description": "Get DeepSeek API usage guide",
                            },
                        ],
                        "prompts": [],
                        "resources": [],
