"""HTTP client for DeepSeek API via AceDataCloud."""
...
from core.exceptions import DeepSeekAPIError, DeepSeekAuthError, DeepSeekError, DeepSeekTimeoutError
...
class DeepSeekClient:
    """Async HTTP client for AceDataCloud DeepSeek API."""
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        if not token:
            ...
            raise DeepSeekAuthError("API token not configured")
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        raise DeepSeekAPIError(message=message, code=code, status_code=status)
    ...
    async def request(
        ...
        Args:
            endpoint: API endpoint path (e.g., "/deepseek/chat/completions")
        ...
        Raises:
            DeepSeekAuthError: If authentication fails
            DeepSeekAPIError: If the API request fails
            DeepSeekTimeoutError: If the request times out
        """
    ...
    async def chat_completions(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return await self.request("/deepseek/chat/completions", kwargs)
